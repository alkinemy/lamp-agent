...
public class AppManagementService {
	...
	public synchronized void register(AppDeployForm form) {
		...
		Exceptions.throwsException(appService.existsApp(id), ErrorCode.APP_ALWAYS_EXIST);

		AppContainer appContainer = JsonUtils.parse(form.getAppContainer(), AppContainer.class);
		AppContext appContext = appContextService.createAppContext(id, appContainer);
		...
		if (appContainer instanceof SimpleAppContainer) {
			...
			if (!simpleAppContainer.isPreInstalled()) {
				...
				appInstallService.install(simpleAppContainer, (SimpleAppContext) appContext, form.getResource());
	...
	public synchronized void deregister(String id, boolean forceStop) {
		...
		if (appContainer instanceof SimpleAppContainer) {
			if (!((SimpleAppContainer) appContainer).isPreInstalled()) {
				...
				appInstallService.uninstall((SimpleAppContainer) appContainer, (SimpleAppContext) app.getAppContext());
